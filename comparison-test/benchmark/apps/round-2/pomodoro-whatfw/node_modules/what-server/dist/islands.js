...
function createIslandStore(name, initialState) {
...
function useIslandStore(name, fallbackInitial = {}) {
  if (sharedStores.has(name)) {
    return sharedStores.get(name);
  }
...
function island(name, loader, opts = {}) {
  islandRegistry.set(name, {
    ...
    // Higher = hydrate first
    stores: opts.stores || []
    // Shared stores this island uses
  });
}
function Island({ name, props = {}, children, mode, priority, stores }) {
...
function processQueue() {
  ...
  Promise.resolve(task.hydrate()).catch((e) => console.error("[what] Island hydration failed:", task.name, e)).finally(() => {
    isProcessingQueue = false;
    queueMicrotask(processQueue);
  });
}
function boostIslandPriority(name, newPriority = 100) {
...
function scheduleHydration(el, entry, props, mode, priority, name, stores) {
  ...
  switch (mode) {
    case "load":
      enqueueHydration({ name, priority: priority + 1e3, hydrate: hydrate2 });
      break;
    case "idle":
      if ("requestIdleCallback" in window) {
        requestIdleCallback(() => {
          enqueueHydration({ name, priority, hydrate: hydrate2 });
        });
      } else {
        setTimeout(() => {
          enqueueHydration({ name, priority, hydrate: hydrate2 });
    ...
    case "action": {
      ...
      const handler = () => {
        ...
        enqueueHydration({ name, priority: priority + 500, hydrate: hydrate2 });
      };
...
function enhance(selector, handler) {
...
function enhanceForms(selector = "form[data-enhance]") {
  enhance(selector, (form) => {
    form.addEventListener("submit", async (e) => {
      ...
      try {
        const csrfMeta = document.querySelector('meta[name="csrf-token"]') || document.querySelector('meta[name="what-csrf-token"]');
        const csrfToken = csrfMeta ? csrfMeta.getAttribute("content") : null;
        const noCsrf = form.getAttribute("data-no-csrf") === "true";
        if (!csrfToken && !noCsrf) {
          console.warn(
            '[what] Form submission blocked: no CSRF token found. Add a <meta name="csrf-token"> tag or set data-no-csrf="true" on the form to opt out.'
          );
        ...
        const response = await fetch(action, {
          method,
          body: method === "GET" ? void 0 : formData,
          headers
        });
